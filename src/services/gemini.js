...
async function loadSystemPrompt(userDisplayName = null) {
  ...
  if (userDisplayName && userDisplayName.trim()) {
    return `${base}\n\nThe user you are talking to is ${userDisplayName.trim()}. In your first message in this conversation, address them by name.`;
  }
  return base;
}
...
export const streamChat = async function* (history, newMessage, imageParts = [], useCodeExecution = false, userDisplayName = null) {
  const systemInstruction = await loadSystemPrompt(userDisplayName);
...
export const chatWithCsvTools = async (history, newMessage, csvHeaders, executeFn, userDisplayName = null) => {
  const systemInstruction = await loadSystemPrompt(userDisplayName);
  const model = genAI.getGenerativeModel({
    model: MODEL,
...
// executeFn(toolName, args) → Promise<result> (generateImage is async)
export const chatWithYouTubeTools = async (history, newMessage, channelDataSummary, executeFn, userDisplayName = null) => {
  const systemInstruction = await loadSystemPrompt(userDisplayName);
  const model = genAI.getGenerativeModel({
    model: MODEL,
  ...
  const chat = model.startChat({ history: chatHistory });
  const msgWithContext = channelDataSummary
    ? `[Channel JSON loaded. Summary: ${channelDataSummary}]\n\n${newMessage}`
    : newMessage;
  ...
  for (let round = 0; round < 8; round++) {
    ...
    if (toolResult?._imageResult) charts.push(toolResult);
